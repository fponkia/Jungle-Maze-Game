...
//               crashing.
//
void testHelperStartup(const std::string &name, int compile_marks,
                       int total_marks);
...
//
template <typename T> bool testHelperIsConst(const T &v) { return true; }
template <typename T> bool testHelperIsConst(T &v) { return false; }
...
bool testHelperConstant(TestType &constant_to_test,
  ...
  if (typeid(CorrectType) != typeid(constant_to_test)) {
    std::cout << "* Incorrect: " << constant_name << " has incorrect type"
              << std::endl;
    return false;
  } else if (!testHelperIsConst(constant_to_test)) {
    std::cout << "* Incorrect: " << constant_name << " is not const"
              << std::endl;
    return false;
  } else if (!(constant_to_test == constant_correct)) {
    std::cout << "* Incorrect: " << constant_name
              << " has incorrect value, should be " << constant_correct
              << std::endl;
    return false;
  } else {
...
//               printed.
//
std::string testHelperEndCapture(bool is_print_captured = true);
...
//               max_digits digits.
//
void testHelperPrintSummaryHeaderPartial(unsigned int indent,
